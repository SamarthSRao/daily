...
export default function HomeDashboard() {
  ...
  const currentWeekNum = Math.ceil(((now.getTime() - new Date(now.getFullYear(), 0, 1).getTime()) / 86400000 + 1) / 7);
    ...
    <div className="meridian-home">
      <div className="meridian-home-header">
        ...
        <div className="meridian-date-area">
          <div>Week {currentWeekNum} of {now.getFullYear()}</div>
      ...
      <div className="meridian-bottom-grid">
        <div className="meridian-weekly-box">
           <div className="meridian-weekly-header">
              <span className="meridian-stat-title">ACTIVE<br/>SPRINT</span>
              <span className="meridian-stat-title" style={{color:"#888", textAlign: "right"}}>{now.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</span>
           </div>
           
           <div style={{ marginTop: "24px" }}>
             <h2 style={{ fontFamily: "var(--meridian-font-serif)", fontSize: "2rem", marginBottom: "8px", color: "var(--text-primary)" }}>
                {activeProject?.title || "No Sprint"}
             </h2>
             <p style={{ fontFamily: "var(--meridian-font-mono)", fontSize: "0.85rem", color: "var(--text-muted)", lineHeight: 1.6 }}>
